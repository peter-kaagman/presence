...
hook before => sub {
    ...
    if (
        ...
        !defined $session_data->{$provider}{id_token}) && request->path !~ m{^/auth}) {
     ...
     #"We zijn blijkbaar aangemeld\n";
     # Maar wat config inladen
     my $sth = database->prepare('Select rowid,* From locaties');
     $sth->execute();
     my $locaties = $sth->fetchall_hashref('naam');
     print Dumper $locaties;
